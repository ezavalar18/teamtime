...
class AsistenciaModel {
    ...
    public function registrarEntrada($id_empleado, $token, $fecha, $hora) {
        ...
        // Registrar entrada con id_tipo_registro = 1
        $stmt = $this->conn->prepare("INSERT INTO registro_asistencia 
                                    (id_empleado, fecha, hora_entrada, id_tipo_registro) 
                                    VALUES (?, ?, ?, 1)");
    ...
    public function registrarSalida($id_empleado, $token, $fecha, $hora) {
        ...
        // Registrar salida y actualizar id_tipo_registro a 1 si no está establecido
        $stmt = $this->conn->prepare("UPDATE registro_asistencia 
                                     SET hora_salida = ?, 
                                         id_tipo_registro = CASE 
                                             WHEN id_tipo_registro IS NULL THEN 1 
                                             ELSE id_tipo_registro 
                                         END
                                     WHERE id_empleado = ? AND fecha = ? 
                                     AND hora_salida IS NULL");
    ...
    public function actualizarHora($tipo, $id_empleado, $fecha, $hora) {
        ...
        $query = "UPDATE registro_asistencia 
                 SET $campo = ?, 
                     id_tipo_registro = CASE 
                         WHEN id_tipo_registro IS NULL THEN 1 
                         ELSE id_tipo_registro 
